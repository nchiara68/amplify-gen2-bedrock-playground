...
export default function EmployeeInformationRetriever() {
  ...
  const [createFaissIndexResponseMessage, setCreateFaissIndexResponseMessage] =
    useState<string>("");
  const [
  ...
  return (
    <Flex direction="column" alignItems="center" padding="1rem" gap="1rem">
      ...
      <Table
        caption="FAISS Index Target Data Location"
      ...
      {createFaissIndexResponseStatusCode !== 0 && (
        <Flex direction="column" alignItems="center" gap="0.5rem">
          <Text>Status Code: {createFaissIndexResponseStatusCode}</Text>
      ...
      <Flex
        ...
        maxWidth="600px"
      >
        <Heading level={2}>Retrieve-Augment-Generate (RAG) Query</Heading>
        <TextField
          label="Your Query"
          ...
          onClick={async () => {
            ...
            try {
              ...
              if (statusCode !== 200) {
                ...
                const maybeMessage = parsedBody?.message || "Request failed.";
                setRagError(`Error ${statusCode}: ${maybeMessage}`);
                return;
              }
              ...
              if (parsedBody?.status === "error") {
                ...
                const errorMessage =
                  parsedBody?.message || "An unknown error occurred.";
              ...
              setRetrievedDocs(parsedBody?.retrieved_docs as RetrievedDocs);
            } catch (error: unknown) {
              if (error instanceof Error) {
        ...
        {Object.entries(retrievedDocs).length > 0 && (
          ...
          >
            ...
            {Object.entries(retrievedDocs).map(([idx, doc]) => (
              ...
              >
                <Text fontWeight="bold">Doc Key: {doc.doc_key}</Text>
